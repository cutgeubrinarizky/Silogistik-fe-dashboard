  ...
  useEffect(() => {
    if (editingAdmin) {
      setFormData({
        ...
        password: '',
        status: editingAdmin.active ? 'active' : 'inactive'
      });
    }
  }, [editingAdmin]);
      ...
      <DialogContent className="max-w-[600px] p-0">
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="phone" className="text-[#0C4A6E]">No. Telepon</Label>
              <Input
                id="phone"
            ...
            <div className="flex items-center space-x-2">
              ...
              />
              <Label htmlFor="status" className="text-[#0C4A6E]">Status Aktif</Label>
            </div>
...
const CourierForm = ({ isOpen, onClose, onSave, editingCourier }) => {
  ...
  useEffect(() => {
    if (editingCourier) {
      setFormData({
        ...
        status: editingCourier.active ? 'active' : 'inactive',
        createAccount: !!editingCourier.email,
  ...
  const handleSubmit = (e) => {
    ...
    const courier = {
      ...
      inProgress: editingCourier?.inProgress || 0,
      ...
      <DialogContent className="max-w-[600px] p-0">
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="plateNumber" className="text-[#0C4A6E]">Nomor Plat</Label>
              <Input
                id="plateNumber"
            ...
            <div className="flex items-center space-x-2">
              ...
              />
              <Label htmlFor="createAccount" className="text-[#0C4A6E]">Buat Akun</Label>
            </div>
            ...
            {formData.createAccount && (
              <>
                ...
                <div className="space-y-2">
                  <Label htmlFor="password" className="text-[#0C4A6E]">
                    Password {editingCourier && '(kosongkan jika tidak ingin mengubah)'}
                  </Label>
                  <Input
                    id="password"
                    type="password"
...
const MarketingForm = ({ isOpen, onClose, onSave, editingMarketer }) => {
  ...
  useEffect(() => {
    if (editingMarketer) {
      setFormData({
        ...
        status: editingMarketer.active ? 'active' : 'inactive',
        createAccount: !!editingMarketer.email,
  ...
  const handleSubmit = (e) => {
    ...
    const marketer = {
      ...
      active: formData.status === 'active',
      deliveries: editingMarketer?.deliveries || 0,
      successful: editingMarketer?.successful || 0,
      failed: editingMarketer?.failed || 0,
      ...(formData.createAccount && {
        email: formData.email,
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            ...
            {formData.createAccount && (
              <>
                ...
                <div className="space-y-2">
                  <Label htmlFor="password" className="text-[#0C4A6E]">
                    Password {editingMarketer && '(kosongkan jika tidak ingin mengubah)'}
                  </Label>
                  <Input
                    id="password"
                    type="password"
...
const EmployeeList = () => {
  ...
  const handleSave = (item) => {
    if (activeTab === "admin") {
      if (editingItem) {
        setAdmins(prev => prev.map(a => a.id === item.id ? item : a));
      ...
      if (editingItem) {
        setCouriers(prev => prev.map(c => c.id === item.id ? item : c));
        toast.success('Kurir berhasil diperbarui');
      } else {
        setCouriers(prev => [...prev, item]);
        toast.success('Kurir baru berhasil ditambahkan');
      }
    } else {
      if (editingItem) {
        setMarketers(prev => prev.map(m => m.id === item.id ? item : m));
        toast.success('Marketing berhasil diperbarui');
      } else {
        setMarketers(prev => [...prev, item]);
        toast.success('Marketing baru berhasil ditambahkan');
      }
    }
  };
  ...
  const handleDelete = (id) => {
    if (activeTab === "admin") {
      ...
      toast.success('Marketing berhasil dihapus');
    }
  };
      ...
      <Table>
        <TableHeader>
          <TableRow className="border-b border-gray-100 hover:bg-transparent">
            <TableHead className="text-[#0C4A6E]/70 font-medium">Nama</TableHead>
            <TableHead className="text-[#0C4A6E]/70 font-medium">Email</TableHead>
            <TableHead className="text-[#0C4A6E]/70 font-medium">No. Telepon</TableHead>
        ...
        <TableBody>
          {filteredAdmins.length === 0 ? (
            ...
            filteredAdmins.map((admin) => (
              <TableRow key={admin.id} className="hover:bg-gray-50/80 transition-all border-b border-gray-100">
                <TableCell className="font-medium text-[#0C4A6E]">{admin.name}</TableCell>
                <TableCell className="text-[#0C4A6E]/80">{admin.email}</TableCell>
                <TableCell className="text-[#0C4A6E]/80">{admin.phone}</TableCell>
                <TableCell>
                  <Badge
                    variant={admin.active ? "success" : "destructive"}
                    className={admin.active ? "bg-green-100 text-green-700 border-green-200" : "bg-red-100 text-red-700 border-red-200"}
                  >
      ...
      <Table>
        <TableHeader>
          <TableRow className="border-b border-gray-100 hover:bg-transparent">
            <TableHead className="text-[#0C4A6E]/70 font-medium">Nama</TableHead>
            <TableHead className="text-[#0C4A6E]/70 font-medium">No. Telepon</TableHead>
            <TableHead className="text-[#0C4A6E]/70 font-medium">Plat Nomor</TableHead>
            <TableHead className="text-[#0C4A6E]/70 font-medium">Status</TableHead>
            <TableHead className="text-[#0C4A6E]/70 font-medium">Akun</TableHead>
            <TableHead className="text-right text-[#0C4A6E]/70 font-medium">Aksi</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {filteredCouriers.length === 0 ? (
            ...
            filteredCouriers.map((courier) => (
              <TableRow key={courier.id} className="hover:bg-gray-50/80 transition-all border-b border-gray-100">
                <TableCell className="font-medium text-[#0C4A6E]">{courier.name}</TableCell>
                <TableCell className="text-[#0C4A6E]/80">{courier.phone}</TableCell>
                <TableCell className="text-[#0C4A6E]/80">{courier.plateNumber}</TableCell>
                <TableCell>
                  <Badge
                    variant={courier.active ? "success" : "destructive"}
                    className={courier.active ? "bg-green-100 text-green-700 border-green-200" : "bg-red-100 text-red-700 border-red-200"}
                  >
                ...
                <TableCell>
                  <Badge
                    ...
                    className={courier.email ? "bg-[#0C4A6E]/10 text-[#0C4A6E] border-[#0C4A6E]/20" : "bg-gray-100 text-gray-600 border-gray-200"}
                  >
      ...
      <Table>
        <TableHeader>
          <TableRow className="border-b border-gray-100 hover:bg-transparent">
            <TableHead className="text-[#0C4A6E]/70 font-medium">Nama</TableHead>
            <TableHead className="text-[#0C4A6E]/70 font-medium">No. Telepon</TableHead>
            <TableHead className="text-[#0C4A6E]/70 font-medium">Alamat</TableHead>
            <TableHead className="text-[#0C4A6E]/70 font-medium">Status</TableHead>
            <TableHead className="text-[#0C4A6E]/70 font-medium">Akun</TableHead>
            <TableHead className="text-right text-[#0C4A6E]/70 font-medium">Aksi</TableHead>
          </TableRow>
        ...
        <TableBody>
          {filteredMarketers.length === 0 ? (
            ...
            filteredMarketers.map((marketer) => (
              <TableRow key={marketer.id} className="hover:bg-gray-50/80 transition-all border-b border-gray-100">
                <TableCell className="font-medium text-[#0C4A6E]">{marketer.name}</TableCell>
                <TableCell className="text-[#0C4A6E]/80">{marketer.phone}</TableCell>
                <TableCell className="text-[#0C4A6E]/80">{marketer.address}</TableCell>
                <TableCell>
                  <Badge
                    variant={marketer.active ? "success" : "destructive"}
                    className={marketer.active ? "bg-green-100 text-green-700 border-green-200" : "bg-red-100 text-red-700 border-red-200"}
                  >
                ...
                <TableCell>
                  <Badge
                    ...
                    className={marketer.email ? "bg-[#0C4A6E]/10 text-[#0C4A6E] border-[#0C4A6E]/20" : "bg-gray-100 text-gray-600 border-gray-200"}
                  >
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-white rounded-xl shadow-[0_0_40px_-10px_rgba(0,0,0,0.1)]" style={{ marginTop: '20px' }}>
          ...
          <div className="px-6 py-6">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center bg-gray-50/80 rounded-lg p-1 gap-1.5">
                <Input 
                  placeholder={`Cari ${activeTab === 'admin' ? 'admin' : activeTab === 'courier' ? 'kurir' : 'marketing'}...`}
                  value={searchQuery}
              ...
              >
                ...
                {`Tambah ${activeTab === 'admin' ? 'Admin' : activeTab === 'courier' ? 'Kurir' : 'Marketing'}`}
              </Button>
            </div>
